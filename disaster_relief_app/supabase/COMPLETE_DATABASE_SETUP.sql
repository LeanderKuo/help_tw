-- =============================================================================
-- DISASTER RELIEF PLATFORM - COMPLETE DATABASE SETUP
-- =============================================================================
...
-- =============================================================================
-- PART 1: DROP EXISTING OBJECTS (Safe to re-run)
-- =============================================================================
...
drop function if exists public.log_role_upgrade_review() cascade;
drop function if exists public.increment_referral_usage(text) cascade;
drop function if exists public.validate_referral_code(text) cascade;
drop function if exists public.redeem_referral_code(text, uuid) cascade;

drop table if exists public.role_upgrade_requests cascade;
...
-- 6) Referral codes (with updated schema)
create table public.referral_codes (
  ...
  created_at timestamptz default now(),
  updated_at timestamptz default now(),
  constraint check_referral_uses check (used_count <= max_uses)
);
...
-- Role upgrade review logging
create or replace function public.log_role_upgrade_review()
returns trigger as $$
begin
  if (old.status = 'pending' and new.status in ('approved', 'rejected')) then
    ...
    values (
      new.reviewed_by,
      case when new.status = 'approved' then 'role_upgrade_approved' else 'role_upgrade_rejected' end,
      'role_upgrade_request',
      new.id,
      jsonb_build_object('user_id', new.user_id, 'from_role', new.from_role, 'to_role', new.to_role, 'reason', new.review_reason)
    );
    if new.status = 'approved' then
      update public.profiles_public set role = new.to_role, updated_at = now() where id = new.user_id;
    end if;
  end if;
  return new;
end;
...
create or replace function public.redeem_referral_code(code_input text, user_id uuid)
returns void as $$
declare
  v_code record;
begin
  ...
  insert into public.audit_logs (actor_id, action, target_type, target_id, meta)
  values (user_id, 'referral_code_redeemed', 'referral_code', v_code.id, jsonb_build_object('code', code_input, 'issuer_id', v_code.issuer_id));
end;
$$ language plpgsql security definer;
...
create trigger trg_log_role_upgrade_review
  ...
  for each row execute procedure public.log_role_upgrade_review();
...
create policy role_upgrade_select on public.role_upgrade_requests
  for select using (auth.uid() = user_id or is_admin_or_above());
create policy role_upgrade_insert on public.role_upgrade_requests
  for insert with check (auth.uid() = user_id and status = 'pending');
create policy role_upgrade_update on public.role_upgrade_requests
